...
## Overview
This package demonstrates ROS2 parameter handling in Python using `rclpy`. It shows how to declare, retrieve, and dynamically update parameters at runtime.

## Purpose
Learn ROS2 parameter management in Python:
...
colcon build --packages-select parameters_py --symlink-install
...
### Running the Node
```bash
ros2 run parameters_py parameters_node
```
...
## Key Concepts
- **Parameter Declaration**: `self.declare_parameter(name, default_value)`
- **Parameter Retrieval**: `self.get_parameter(name).value`
...
## Files
- `parameters_node.py` - Parameter handling node implementation
