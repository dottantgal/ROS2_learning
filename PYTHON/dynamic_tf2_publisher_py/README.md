# dynamic_tf2_publisher_py

## Overview
This package demonstrates dynamic TF2 transformation publishing in Python using `rclpy` and `tf2_ros`. It allows runtime reconfiguration of transformation parameters without rebuilding.

## Purpose
Learn TF2 transformation publishing in Python:
...
colcon build --packages-select dynamic_tf2_publisher_py --symlink-install
...
```bash
ros2 run dynamic_tf2_publisher_py tf2_publisher_node
```
...
- `tf2_publisher_node.py` - Node entry point
